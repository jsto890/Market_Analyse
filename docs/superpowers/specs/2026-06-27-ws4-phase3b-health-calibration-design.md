...
- **Data.** Daily OHLCV per name via the existing `get_history`/yfinance path, ~2014→present (covers ≥3 distinct regimes incl. the 2022 bear). Cache once to `argus/backtests/_corpus/<ticker>.parquet` (gitignored). Re-runs read the cache → offline, repeatable. Names with insufficient history or fetch failures are skipped and logged; the skip list is part of the run artifact.
...
- **Weights from a shrunk multivariate fit.** Fit a **ridge-regularized (L2) model** of the binary adverse label on the (graduated) signal indicators; the shrunk coefficients de-correlate the overlapping price signals (H1/H2/H4) and give marginal — not double-counted — contributions. Non-graduating signals are fixed at weight 0. Interpolate the resulting weight vector toward equal weights by a shrinkage factor λ tuned **inside the training fold only**. Renormalise the live signals' weights to a fixed composite scale.
