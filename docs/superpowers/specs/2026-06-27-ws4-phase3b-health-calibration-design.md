...
- **Label (pre-registered).** For each in-position trade-day *t*, the **forward 20-day max adverse excursion (MAE)**: the largest drawdown from the close at *t* over the next **20 bars (fixed forward window)**, expressed in **ATR(14) units** at *t*. A binary companion label `adverse = MAE ≥ k·ATR` (k pre-registered, e.g. 1.5) supports AUC/Brier scoring. **Window: FIXED 20 bars, NOT capped at exit.** An earlier draft capped at `min(20, bars_to_actual_exit)`; direct evidence (30-name diagnostic, 2026-06-29) showed this reintroduces the F3 time-to-exit **censoring**: deterioration flags fire late in a hold → short capped window → mechanically small MAE → spurious negative IC (H2 fires ~11 bars before exit vs ~21 for non-firing bars; capping inflates its negative IC ~6×). Because health is alert-only (decoupled from P&L), the coherent predictive estimand is the stock's forward downside over a fixed horizon, independent of our exit — so the window is fixed (`panelbuild.build_panel(cap_at_exit=False)` default). The held-position variant remains available behind `cap_at_exit=True`.
- **Per-signal skill.** For each signal Hᵢ (computed point-in-time at *t* from 3a's `health.py`), estimate its association with the forward label: per-day Spearman rank-IC of the flag/score against MAE (averaged over days), and AUC against the binary label (descriptive). **Graduation gate (3b-2 implementation):** a signal graduates iff its mean per-day rank-IC is in the deterioration direction (>0) **and** its **one-sided within-day permutation p-value** (2000-shuffle null, §7) clears **Holm-Bonferroni** across the (≤5) signals. The permutation null is the operationalisation of "skill positive with a CI excluding zero after Holm" — it yields genuine per-signal p-values so Holm actually controls family-wise error (a binary CI-pass proxy makes Holm a no-op), and it is robust to the heavy ties of binary signals.
- **Cluster bootstrap (F5).** A **descriptive** CI of the mean per-day rank-IC, by resampling **whole trading-day cross-sections** (all names on a sampled date move together) in **fixed-length moving blocks of 30 trading days** (Künsch; ~20–40d range per §6), with a **by-name cluster** resample as a robustness check. This preserves the dominant contemporaneous + serial correlation, unlike a time-only or per-observation bootstrap. It **abstains (reports NaN)** when valid days < 3·block_days, since below that every replicate is identical and the CI degenerates to a zero-width point mass. The graduation decision rests on the permutation p-value above, not on this CI.
