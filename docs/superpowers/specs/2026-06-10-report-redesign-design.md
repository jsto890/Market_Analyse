...
**Date:** 2026-06-10
**Scope:** Collapse the two daily reports into **one** report, generated by `Market_Analyse/sentiment_bridge.py`. The Twitter Memo is no longer produced as a separate file; its one surviving section (Sector Rotation) becomes the top section of the single report. Goal: cut noise, surface only what's used for picking long candidates, and make every column/line mean something.

---

## Goal

The single report is read each morning to (a) see where chatter is rotating and (b) pick long candidates. Strip everything that doesn't serve those two jobs. Make it explainable — show *why* a ticker is classified LONG so it can be sanity-checked rather than trusted blindly.

## Single-report structure (top to bottom)
1. **Sector Rotation** — macro context (where chatter is rotating).
2. **Aligned** table — all three legs bullish.
3. **Technical + Fundamental** table.
4. **Long Candidate Detail** — per-ticker evidence blocks.
5. Footer note (entry/stop/target deferral).

## Pipeline change
`run_daily.sh` stops rendering and copying the Twitter Memo to Obsidian. The X fetch / extract-signals / classify-setups steps remain (they feed the sentiment leg and the setups data that drives Sector Rotation). Only one file lands in Obsidian: `YYYY-MM-DD <single report>.md`. The Sector Rotation logic moves out of `memo.py` into a module the bridge calls (shared with the taxonomy resolver), reading the full `ticker_setups.csv` universe.
...
## Section 1 — Sector Rotation (top of the single report)

The Twitter Memo's only surviving section. The memo's other sections are gone (not relocated): Top Talked-About Stocks, Reliable Account Chatter, News That Can Move Stocks, Areas To Watch, Account Leaderboard, Account Trust, Theme Map, Action Queue, Changed Since Last Run, Needs Confirmation, Data Freshness, Actionability Notes, setup sections. The standalone Twitter Memo file is no longer written.

**Sector Rotation** — grouped by Family, sub-sector is the rotation row. Built from the full `ticker_setups.csv` universe (all classified tickers, not just the ~64 analysed by the bridge). Rotation direction reuses the existing setup-label proxy (no time series needed):
...
## Sections 2-4 — Sentiment × Technical × Fundamental (long candidates)

These sections follow Sector Rotation in the same file. All three legs share the same −1..+1 signed scale (`sentiment_score`, `tech_score`, `catalyst_score`).
