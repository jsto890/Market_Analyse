...
1. **Discord ingest** (`argus/news/discord_ingest.py`): a small `discord.py` self-contained client with a **read-only bot token**, listening to the user's financial-news bot channel(s); parses messages (headline embeds, the daily Market Report format from the screenshot — futures snapshot, headlines list, "What to Expect Today" economic table, earnings line) into `news_items` + `econ_calendar`. This is a **different process model from every other job** — a persistent gateway websocket, not a scheduled batch. It runs as a supervised long-lived service (`launchd KeepAlive=true`), and its reconnect contract is explicit: on every (re)connect it **backfills channel history since the last stored item for that source**, so sleep/disconnect windows self-heal instead of silently dropping messages. Token handled per the secrets rule (§2.2.6). **Needs from user:** create a bot token in the Discord developer portal, invite it to the server with read-message scope, provide channel IDs. (~10 min, instructions will be provided at implementation.)
...
4. **Morning macro report:** generated 8:00 AM ET from `macro_sentiment` + rail quotes + `econ_calendar` + earnings — rendered as the dashboard's landing header block and appended to the daily Obsidian report. Format mirrors the Discord bot screenshot (futures, one-paragraph read, headlines, what to expect, earnings).
5. **Whale alerts:** the relative-unusual scorer (WS-1) already finds them per-symbol; whale alerts = cross-market top-N by **premium traded** (vol × last × 100) filtered to score > threshold, streamed into `news_items` with `source=whale` so they appear inline in the news rail with a 🐋 chip. v1 scans only tracked symbols (snapshot universe); full-market sweep scanning needs a paid flow feed (open question Q5).
...
| Q5 | **Whale alerts scope**: v1 = tracked-symbol universe (free). Full-market sweep detection needs a paid flow feed (UnusualWhales API, CBOE feeds). Pay or defer? | WS-3 | Defer; evaluate after v1 proves the display is useful |
| Q6 | **Execution host** — the laptop sleeps during US hours (§2.4). v1 mitigates with `pmset` wakes + backfill, but paper/live automation (and gap-free snapshots) really want an always-on box. Mac mini / VPS budget? | **Phase B-0** (elevated from "later" by architecture review) | `pmset` + backfill for Phases A–E; commit to an always-on host as a Gate-2 prerequisite |
| Q7 | **Risk rails for automation** (max positions, % per trade, daily loss cutoff, in-flight-order kill policy) | WS-7 Gate 2 | Propose at Gate 2 review with backtest-derived numbers |
| Q8 | **0DTE GEX limitation**: OI-based gamma can't see intraday 0DTE flow (§WS-1.5). Accept labelled ≥1DTE levels in v1, or fund a paid intraday-flow feed for true 0DTE gamma? | WS-5 | Accept the labelled limitation in v1; revisit with Q5's flow-feed evaluation |
| Q9 | **IBKR paper account** exists and Gateway can run during US hours? | WS-7 Gate 2 | Confirm before Gate 2 work starts |
