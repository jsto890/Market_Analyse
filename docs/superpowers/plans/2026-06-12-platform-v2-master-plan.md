...
2. **Relative-unusual scorer** — replaces the naive same-day volume filter. Quant review rejected the first-draft design (`vol/OI` z-scores vs 10 adjacent strikes) as statistically unsound: `vol/OI` explodes on tiny-OI contracts, raw adjacent strikes mix incomparable moneyness levels, and mean/std z-scores on heavy-tailed counts flag the low-OI tail constantly — trading "too empty" for "too noisy". **Adopted design:**
   ...
   - **MAD=0 rule (quant-adjudicated 2026-06-13, WS-1 build):** when MAD=0 (tie-heavy small samples are routine for `log1p(count)` data), fall back to the baseline's **sample std-dev** as the scale; if that is also 0 (truly constant baseline) or the baseline has <3 points, **suppress the term** (return None, like "insufficient history") rather than scoring 0 — `score = max(...)` over present terms only. The earlier "MAD=0 → z=0" rule had a blind spot (a 250× outlier vs identical neighbours scored 0); a median-scaled fallback was rejected as level-dependent and indefensible.
...
**Multi-timeframe ensemble (core model design):** the model is built from technical signals computed on **multiple timeframes simultaneously** (weekly / daily / intraday as data allows), and the optimisation phase searches for the best *combination* of signals across timeframes — e.g. weekly trend filter × daily entry trigger × intraday confirmation — producing one overarching state, not parallel per-timeframe verdicts. Prior art in this repo already validates the idea: PRIME_LONG's weekly-confirm requirement moved win rate from 35.7% → 55.3%. **Overfitting warning applies double here:** a cross-timeframe combination search multiplies degrees of freedom, so every candidate combination is subject to the same OOS discipline below — the search runs on training folds only, and the chosen combination is frozen before touching validation data.

**Backtest harness** (`argus/position_engine/backtest.py`) — **a development and optimisation tool, full stop.** It runs in the background while the model is being built and tuned; it is *not* a dashboard feature and gets no UI (consistent with the 2026-06-08 decision to remove the backtest page). It runs the full state machine over any ticker list × any period × any candle interval (1d first; 1h/15m later), producing per-trade logs (entry/exit ts+px, R, MAE/MFE, holding days) and aggregate stats (WR, avg R, expectancy, exposure, max DD, vs buy-and-hold of the same ticker, vs SPY). Quant review of the inherited `tools/backtest/` machinery found two leakage vectors and a survivorship problem that are now **hard rules**:
...
- **Engine decision — RESOLVED (2026-06-22, Phase 2 plan).** Reuse the Phase-1 `replay()` runner, **not vectorbt** (not installed; the machine is stateful — Schmitt/dwell/overlay — so vectorisation fights it). The 8 pre-registered tunables are injected via a frozen `EngineParams` threaded through the pure fns + `replay`, defaulting to today's constants (126 tests stay green). A separate backtest cost/fill layer re-prices each exit — **IBKR 1-hour bars for exact multi-TF fills/sequencing where available, conservative daily fallback (gap-through stops, next-open targets, stop-first straddles) elsewhere**. Metrics are R-space (sizing-free, matching the signals-only engine). Backtest sweeps write to per-run files under `argus/backtests/<ts>/`, never the live DB (§2.2.4). See [2026-06-22-ws4-phase2-backtest-harness.md](2026-06-22-ws4-phase2-backtest-harness.md).
...
**On-demand model run (the dashboard surface):** the ticker page gets a **"Run model"** button (same on-demand pattern as the AI panel — never auto-fires). Click → Argus endpoint runs the state machine over that ticker's loaded history → the chart renders **green up-arrows below entry candles and red down-arrows above exit candles**, with hover detail (state, entry/stop/target at that bar, resulting R), plus a summary line above the chart: trades, hit rate and avg R *on this ticker*, current state, and model confidence. Results cached per (ticker, model_ver, data-date) so repeat clicks are instant. For tickers already in the live signal stream, the same markers render from `position_signals` history without needing a run. This is the "when clicking into a ticker we can see when we said to buy and sell".
...
| C / WS-3 | WS-3 news/macro | **Done** — all four slices merged: 3a news pipeline, 3b macro FinBERT, 3c econ calendar, 3d morning-report + whale alerts | [3a](2026-06-16-phase-c-ws3a-news-pipeline.md) · [3b](2026-06-16-phase-c-ws3b-macro-sentiment.md) | 2026-06-22 | Merged to main; live launchd jobs (news-ingest, macro aggregator 20min, morning brief 8am ET) running |
| D / WS-5 SP1 | WS-5 0DTE hub — vendor + embed (SP1) | **Done** — `odte/` vendored (upstream `f7c1240`), backend venv + prebuilt dist committed, launchd `com.argus.odte` on `:8788`, `/odte` iframe page + health badge, nav link, smoke pass (9/9) | [plan](2026-06-30-ws5-sp1-odte-vendor-embed.md) | 2026-07-03 | Merged to main (`aff923e`). QQQ ladder live at `/odte`. |
| D / WS-5 SP2a | WS-5 0DTE hub — ETF symbol switch (SP2a) | **Done** — in-process switch across SPY/QQQ/IWM/DIA. Upstream `SWITCHABLE_SYMBOLS` + `POST /control/symbol` (OptionsAnalysis `b448682`), re-vendored into `odte/`, dashboard symbol proxy + 4-button `/odte` selector. Suites green, smoke 9/9, live switch verified on `:8788`. | [plan](2026-06-30-ws5-sp2a-etf-multi-underlying.md) | 2026-07-06 | Merged to main (`6765bc8`). Boot default QQQ, no persistence. |
| D / WS-5 SP3 | WS-5 0DTE hub — companion 2×2 WS-1 grid (SP3) | **Done** — GEX/unusual/P-C/spot cards under the `/odte` ladder, keyed to active symbol. New argus `GET /api/pcr/{symbol}` (latest-kind aggregation), 3 dashboard proxies to `:8088`, index→ETF companion mapping ships now (SPX→SPY etc., PROXY chip). argus 236 + vitest 76 + smoke 9/9 green. | [plan](2026-07-06-ws5-sp3-companion-grid.md) | 2026-07-16 | Merged to main. Soak discharged by SP2b's 2026-07-16 run (PASS); argus plist FD limit raised to 4096 (installed copy only). |
| D / WS-5 SP2b | WS-5 0DTE hub — index underlyings (SP2b) | **Done** — SPX/NDX/RUT/DJX via `Index()` contracts + trading-class chain filter (SPXW/NDXP/RUTW/DJX), strike-step inference, 8-symbol two-group selector. Upstream `331b738` re-vendored. Backend 52 + vitest 78 green. 60-min soak PASS (240/240 polls, pid stable, SPX/RUT/DJX refill live). **Known limitation: NDX ladder empty — IBKR account lacks NASDAQ index (Network C) market-data entitlement (Error 354); code path proven on other indexes, lights up once entitled.** Subs counter cumulative (cosmetic upstream quirk). | [plan](2026-07-06-ws5-sp2b-index-underlyings.md) | 2026-07-16 | Merged to main. WS-5 complete. |
| E / WS-4 P1 | WS-4 Position Engine — core engine | **Done** — two-axis bias+overlay state machine, 4-table schema, pure fns + `replay()` runner; 126 tests green | [plan](2026-06-17-ws4-phase1-core-engine.md) · [spec](../specs/2026-06-17-ws4-position-engine-design.md) | 2026-06-17 | Merged to main (`5808396`). |
| E / WS-4 P2 | WS-4 Position Engine — backtest harness + corpus validation | **Done** — FAIL pre-registered bar. 608 names / 10,525 trades / 11yr. Winsorized exp +0.062R [CI +0.011, +0.116] at 1× cost; MAR 0.13 vs SPY 8.57; evaporates at 2×/3×. Edge = SELECTION. Robust stats fix (R_CLIP=25) merged separately. | [plan](2026-06-22-ws4-phase2-backtest-harness.md) | 2026-07-03 | Merged to main (`78b95fe`). WS-7 Gate 1 not cleared. |
| E / WS-4 P3 | WS-4 Position Engine — health signals + evaluator | **Done (closed)** — Phase 3a health signal architecture complete; signals anti-predictive (contrarian buyable-dip markers). Phase 3b predictive evaluator: signals anti-predictive → ship 1/N weights. Health dormant — do not surface or trade. | [3a plan](2026-06-27-ws4-phase3a-health-monitor.md) · [3b-1](2026-06-27-ws4-phase3b1-corpus.md) · [3b-2](2026-06-27-ws4-phase3b2-evaluator.md) | 2026-07-03 | Merged to main. |
| E / WS-4 exit | WS-4 Exit premise-check | **Done — NO-GO** — oracle ceiling ~1.1R; 5-rule family (giveback_trail, chandelier, donchian, no_progress, profit_target_3R); 907 OOS trades; best p=0.44; hold-to-structural-stop unbeatable. | [plan](2026-06-29-ws4-exit-premise-check.md) | 2026-07-03 | Merged to main. Edge stays SELECTION, not exit geometry. |
| E / WS-4 notifier | WS-4 Daily action notifier | **Done** — notify-only, ENTRY/EXIT/TRAIL diffs, runs as launchd job. No orders. | [plan](2026-06-30-ws4-daily-action-notifier.md) | 2026-07-03 | Merged to main. |
| F | WS-7 automation | **Gated** — Gate 1 (P2 pass) not cleared; bar not met. Next lever: gap-continuation entry archetype experiment. | — | 2026-07-05 | Gate 1 needs a revised engine clearing MAR vs SPY; VPS (Q6); risk rails (Q7). |
