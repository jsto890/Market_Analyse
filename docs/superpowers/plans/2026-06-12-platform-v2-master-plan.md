...
| E / WS-4 P1 | WS-4 Position Engine — core engine | **Done** — two-axis bias+overlay state machine, 4-table schema, pure fns + `replay()` runner; 126 tests green | [plan](2026-06-17-ws4-phase1-core-engine.md) · [spec](../specs/2026-06-17-ws4-position-engine-design.md) | 2026-06-17 | Merged to main (`5808396`). Known gaps deferred by design: `health=None`, `trail_stop` defined but unwired |
| E / WS-4 P2 | WS-4 Position Engine — backtest harness | **Plan written, ready to execute** — reuse tested `replay()` (not vectorbt), 8-param `EngineParams` sweep, IBKR-1h multi-TF exact-fill model + daily fallback, R-space metrics, `shuffle_future` leakage gate + pre-registered MAR/trades success-bar | [plan](2026-06-22-ws4-phase2-backtest-harness.md) | 2026-06-22 | Executing (subagent-driven, multi-agent with review). User to expose IBKR API on port 7496 (`IBKR_PORT=7496`, read-only) for intraday fills. Task 2 (wire trail) = REQUIRED, runs first (trail is baseline per §5/§95) |
