...
> **Trails unblocked and built 2026-08-01.** `sector_rotation.py` now writes `reports/rotation_history.json` alongside the snapshot — date → industry → `[rs_ratio, rs_mom]`, the last 70 daily runs, keyed by date so a re-run corrects a day rather than adding a second point to it. `app/rotation/page.tsx` reads it and `lib/rotationTrails.ts` collapses it to **one point per ISO week, at most 8** (`weeklyTrail`): the job runs daily, but daily points would draw the sampling noise rather than the rotation. A sector with fewer than two weeks gets no trail, and the panel subtitle stays silent until there is one — so the chart is a scatter plot for its first week and grows tails from there. The trail extents are folded into the axis domain; without that, the weeks that led to today's dot were clipped at the plot edge. No endpoint was added: the page is a server component reading the same directory as the snapshot, so an HTTP hop would buy nothing. **The history starts 2026-08-01; the first tail appears the week of 2026-08-10.**
>
