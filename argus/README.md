...
| **Period returns** | Trailing 1D / 1W / 1M / 6M / 1Y price return per ticker, surfaced as a colour-coded graphic in the dashboard and reports. |
...
| **REST API** | FastAPI app on `127.0.0.1:8088`. ~24 routes. |
| **Minimal UI** | One static HTML page (no framework, no build) with 6 tabs. Everything calls the REST API. |
| **MCP server** | FastMCP server over **stdio** that exposes ~18 tools (`argus_*`) so Claude Desktop, Cursor, etc. can drive the platform directly. |
...
Expected output: indicators computed, agents registered, action card built, FastAPI app + MCP server build.
...
Restart Claude Desktop. You'll see ~18 `argus_*` tools (`argus_action_card`, `argus_screen`, `argus_portfolio`, `argus_chart_chat`, `argus_place_bracket_order`, etc.).
...
Everything else — the 52-agent ensemble, the Action Card, the screener, the period-return panel, the portfolio edge overlay, the alerts, the FastAPI server, the MCP server — runs end-to-end on your machine.
